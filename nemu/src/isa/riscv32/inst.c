...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // implement leap-year.c
  
  // implement load-store.c
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));

  // implement matrix-mul.c

  // implement max.c
  
  // implement mersenne.c
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   ,
          R, R(rd) = ((int64_t)((int64_t)src1 * (int64_t)src2)) >> 32);
  ...
  // implement mul-longlong.c

  // implement switch.c // to-lower-case.c
